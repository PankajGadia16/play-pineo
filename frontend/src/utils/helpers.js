...
export default {
    ...
    getTeamIdFromPlayerId: function (playerId) {
        return ["player1", "player2"].includes(playerId) ? "teamA" : "teamB"
    },
    isValidBunch: function (cards) {
        let suit = null
        let rank = null
    ...
    getCardRank: function (cardString) {
        return parseInt(cardString.slice(0, cardString.length - 1))
    },
    getUpdatedBunch: function (oldBunch, entry) {
    ...
    calculateScores: function (game) {
        ...
        return {
            teamA: (15 * teamASets) + (5 * teamAPureSets) + this.totalScoreOFCards(teamACards) - this.totalScoreOFCards((player1Cards.concat(player2Cards))),
            teamB: (15 * teamBSets) + (5 * teamBPureSets) + this.totalScoreOFCards(teamBCards) - this.totalScoreOFCards((player3Cards.concat(player4Cards)))
        }
    },
    totalScoreOFCards: function (cards) {
        let score = 0
        for (let card of cards) {
            if (card == "PINEO") {
                score += 4
    ...
    getNextTurn: function (currentTurn) {
    ...
    sortCards: function (cards) {
        ...
        const spadeCards = cards.filter(card => card[card.length - 1] == "S").sort((a, b) => this.getCardRank(a) - this.getCardRank(b))
        const heartsCards = cards.filter(card => card[card.length - 1] == "H").sort((a, b) => this.getCardRank(a) - this.getCardRank(b))
        const clubsCards = cards.filter(card => card[card.length - 1] == "C").sort((a, b) => this.getCardRank(a) - this.getCardRank(b))
        const diamondCards = cards.filter(card => card[card.length - 1] == "D").sort((a, b) => this.getCardRank(a) - this.getCardRank(b))
